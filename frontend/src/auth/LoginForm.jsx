...
const LoginForm = ({ onClose, switchToSignup, onResumeKyc }) => {
  ...
  // Separate from the inline `error` div below — a blocked account gets
  // its own InfoModal instead, since this is a harder stop than a normal
  // form validation error and deserves a more deliberate "OK" dismissal
  // rather than sitting in the same small red text block as e.g. a typo'd
  // password.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // auth_routes.py's login route raises this exact 403 message for a
      // blocked account — distinct from the KYC-gate 403 below, which
      // uses "Login not allowed. KYC status: ..." instead.
      if (err?.response?.status === 403 && typeof rawDetail === "string" && rawDetail.includes("blocked")) {
      ...
      if (err?.response?.status === 403 && typeof rawDetail === "string") {
        if (rawDetail.includes("rejected")) {
          setKycState("rejected");
        } else if (rawDetail.includes("pending")) {
          setKycState("pending");
        }
      }
    } finally {
  ...
  const handleResumeVerification = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(err, "Could not resume verification"));
    } finally {
      setResuming(false);
    }
  };
    ...
    <div className="relative mx-4 w-full max-w-xs bg-neutral1 p-6 shadow-lg sm:mx-0 sm:max-w-md sm:p-8 md:max-w-lg">
      ...
      {showBlockedModal && (
        <InfoModal
          ...
          message="Your account has been blocked by an admin. Contact support if you believe this is a mistake."
          primaryLabel="OK"
          onPrimary={() => setShowBlockedModal(false)}
        />
      )}
    </div>
  );
